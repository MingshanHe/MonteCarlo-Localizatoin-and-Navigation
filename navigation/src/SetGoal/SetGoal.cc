...
public:
    ...
    void push_command(void)
    {
        ...
        int a = 0;
        int b = 4;
        std::list<int> command {up,down,left,right};
        std::list<int> command_pro = {stay,stay,stay,stay,stay,stay,stay,stay};
        ...
        for (it = command.begin();it!=command.end();it++){
            if(*it == 1)
            {
                ...
                {
                    command_pro.pop_back();
                    command_pro.push_front(*it);
                }
            }
            else if(*it == 2)
            {
        ...
        i = (rand() % (b-a+1))+ a;
        int i_ = 0;
        for (it = command_pro.begin();it!=command_pro.end();it++)
        {
            if (i == i_)
            {
                command_word = *it;
    ...
    void get_value_by_command(void)
    {
        if(command_word == 0)
        {
            x = x;
