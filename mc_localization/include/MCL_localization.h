...
class PFLocalization
{
	public:
        ...
        PFLocalization(map_type* mapdata,Odom_Data new_odom_data,Odom_Data old_odom_data,vector<double> data,vector<log_data>& logdata,ros::NodeHandle node);
        ~PFLocalization();
		void InitParticles(bool flag);
		void MCLAlgorithm();
		void CalRobotPose();
		void Visualize();
