...
PFLocalization::PFLocalization(map_type* mapdata,Odom_Data new_odom_data,Odom_Data old_odom_data,vector<double> data,vector<log_data>& logdata,ros::NodeHandle node)
{

	this->data = data;
	this->new_odom_data = new_odom_data;
	this->old_odom_data = old_odom_data;
...
//粒子集初始化函数
void PFLocalization::InitParticles(bool flag)
{
	int count = 1;
...
void PFLocalization::MCLAlgorithm()
{
	...
	motion_.x_front = old_odom_data.x_robot;
	motion_.y_front = old_odom_data.y_robot;
	motion_.theta_front = old_odom_data.theta_robot;
	motion_.x_rear = new_odom_data.x_robot;
	motion_.y_rear =new_odom_data.y_robot;
	motion_.theta_rear = new_odom_data.theta_robot;	
	...
	// measurement_.readings = data.readings;
...
float PFLocalization::MeasurementBeamModel(particle_state particle)
{
	...
	{
		zkt = data[i];		//单位：dm,从log文件读取时就已经转换成dm了
...
float PFLocalization::MeasurementScoreModel(particle_state particle)
{
	...
	for (int i = 0; i < data.size(); i++)
	{
		zkt = data[i];		//第i个激光束的测距，单位：dm,从log文件读取时就已经转换成dm了
